...
public class AtomStructureAPITest {
  ...
  public void testGetLogicalDistance() throws IOException {
    ...
    Electron e1 = tracks.get(0).ObjectOnTrack.get(1);
    Electron e2 = tracks.get(2).ObjectOnTrack.get(13);
  ...
  public void testGetDifference() throws IOException {
    system = ConstractASystem.Constract("src/TXT/AtomicStructure.txt");
    ConcreteCircularOrbitE system1 
        = ConstractASystem.Constract("src/TXT/AtomicStructure_Medium.txt");
    Difference difference = caculate.getDifference(system, system1);
    assertEquals(1,difference.getTrackNum());
    List<Integer> num = difference.getObjectNum();
    assertEquals(-22,(int)num.get(3));
    List<Electron> object = difference.getObject1();
    assertEquals(true,object.contains(new Electron(3)));
  }
