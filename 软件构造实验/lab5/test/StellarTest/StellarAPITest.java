...
public class StellarAPITest {
  ...
  public void testGetDifference() throws IOException {
    system = ConstractAStellarSystem.constract("src/TXT/StellarSystem.txt");
    ConcreteCircularOrbit system1 = 
        ConstractAStellarSystem.constract("src/TXT/StellarSystem_Medium.txt");
    Difference<PlanetWithSatellite> difference = caculate.getDifference(system, system1);
    assertEquals(1606,difference.getTrackNum());
    List<Integer> objectNum = difference.getObjectNum();
    assertEquals(0,(int)(objectNum.get(0)));
    List<PlanetWithSatellite> list = difference.getObject1();
