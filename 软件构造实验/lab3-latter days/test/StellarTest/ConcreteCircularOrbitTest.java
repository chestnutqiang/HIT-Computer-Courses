...
public class ConcreteCircularOrbitTest{
	...
	public void testAddObjectOnTrack() throws IOException {
		Planet p=new Planet(new label("w"),new label("x"),new label("w"),new number("5"),new number("2"),new number("0"),"CW",0.1314);
	...
	public void testCurrentAngle() throws IOException {
		...
		assertEquals(0,system.CurrentAngle(list.get(0).ObjectOnTrack.get(0),t).compareTo(new BigDecimal("207")));
		assertEquals(0,system.CurrentAngle(list.get(1).ObjectOnTrack.get(0),t).compareTo(new BigDecimal("88")));
		assertEquals(0,system.CurrentAngle(list.get(2).ObjectOnTrack.get(0),t).compareTo(new BigDecimal("251")));
		assertEquals(0,system.CurrentAngle(list.get(3).ObjectOnTrack.get(0),t).compareTo(new BigDecimal("9")));
		assertEquals(0,system.CurrentAngle(list.get(4).ObjectOnTrack.get(0),t).compareTo(new BigDecimal("77")));
		assertEquals(0,system.CurrentAngle(list.get(5).ObjectOnTrack.get(0),t).compareTo(new BigDecimal("109")));
	}
	
	@Test
	public void testDistanceOfTwoPlanet() {
		system=new ConcreteCircularOrbit();
		Planet p=new Planet(new label("w"),new label("x"),new label("w"),new number("5"),new number("37"),new number("0"),"CW",0.1314);
		Planet p1=new Planet(new label("w"),new label("x"),new label("w"),new number("7"),new number("46"),new number("9"),"CW",90.1314);
		PlanetWithSatellite P=new PlanetWithSatellite(p),P1=new PlanetWithSatellite(p1);
